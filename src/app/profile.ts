...
- Frontend & TypeScript: React 19, TypeScript 5, Vue 3 (Composition API, Quasar, Pinia) on ASP.NET Core in production at RUBICON. Angular: professional experience from enterprise client work that is under confidentiality and can't be named publicly (earlier work; the recent professional frontend has been React and Vue) — and he still ships Angular today: the full TourPlanner SPA is Angular (typed OpenAPI client, Vitest, 259 frontend tests) against a .NET 10 API, Dockerised, and this portfolio site itself is his own Angular v22 SSR app (Express, streaming AI assistant) running in production. If asked whether the Angular experience is professional: yes — client work he can't reference by name; state that plainly, without apology and without inflating it. Blazor Server + FluentUI; WPF (.NET 8). Shipped live frontends: atelierbella.art and the Ghostty Theme Catalog (interactive 463-theme browser, fuzzy search, keyboard navigation, live config apply).
